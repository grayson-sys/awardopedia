...
def fetch_page(date_start: str, date_end: str, page: int) -> dict:
    payload = {
        "filters": {
    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2:
                raise
            log.warning(f"Page {page} attempt {attempt+1} failed: {e} — retrying in 5s")
            time.sleep(5)
...
def map_row(r: dict, fetched_at: str) -> dict:
    """Map a USASpending API row to our DB schema."""
    ...
    # Auto-generate keywords from description + PSC description
    ...
    return {
        ...
        "recipient_uei":     r.get("Recipient UEI") or r.get("Recipient DUNS"),
        ...
        "pop_county_fips":   pop_county_fips,
        "pop_county_name":   pop_county_name,
        "pop_city_name":     pop_city,
        "pop_zip":           pop_zip,
        # Source attribution — the trust layer
        "source_url":        build_source_url(award_id, internal_id),
        "solicitation_url":  build_solicitation_url(solicitation),
        "source_system":     SOURCE_SYSTEM,
        "source_fetched_at": fetched_at,
        "confidence":        CONFIDENCE,
        # Jurisdiction (federal for all USASpending data)
        "jurisdiction_type": "federal",
        "jurisdiction_fips": "US",
        # Keywords (basic; AI enrichment runs separately)
...
UPSERT_SQL = """
INSERT INTO awards (
  award_id, recipient_name, recipient_uei, amount, start_date, end_date,
  description, awarding_agency, awarding_sub_agency, funding_agency,
  naics_code, naics_description,
  psc_code, psc_description, sector_slug, contract_type,
...
ON CONFLICT (award_id) DO UPDATE SET
  ...
  pop_county_name   = COALESCE(EXCLUDED.pop_county_name, awards.pop_county_name),
...
def run(date_start: str, date_end: str, dry_run: bool = False):
    ...
    while True:
        ...
        if meta.get("last_page", True) or page >= meta.get("num_pages", 1):
            break
